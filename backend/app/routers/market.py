...
_ERROR_CACHE_TTL_SECONDS = 20  # briefly cache failures too, so a burst of "Try again" clicks doesn't pile more requests onto an already-limited upstream
_cache: dict[str, tuple[float, int, dict]] = {}


async def _proxy_get(url: str, cache_ttl: int = _CACHE_TTL_SECONDS, headers: dict | None = None) -> JSONResponse:
    cached = _cache.get(url)
    if cached:
        cached_at, status_code, body = cached
        ttl = cache_ttl if status_code == 200 else _ERROR_CACHE_TTL_SECONDS
...
async def crypto_markets(request: Request):
    ...
    # CoinGecko's free/no-key tier has the tightest shared rate limit of
    # everything this app calls, so cache it longer than the 60s default.
